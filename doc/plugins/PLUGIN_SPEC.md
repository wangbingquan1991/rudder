...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Rudder app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Rudder HTTP APIs directly.
...
Rudder plugin design is based on the following assumptions:

1. Rudder is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Organizations" remain core Rudder business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Rudder core.
...
2. Let plugins add major capabilities without editing Rudder core.
...
4. Depend on project-local plugin folders such as `.rudder/plugins`.
...
The single Rudder deployment an operator installs and controls.

### 5.2 Organization

A first-class Rudder business object inside the instance.
...
A trusted in-process extension loaded directly by Rudder core.
...
An installable instance-wide extension package loaded through the Rudder plugin runtime.
...
Rudder has two extension classes.
...
Rudder already has a concrete workspace model:
...
There is no per-organization install table and no per-organization enable/disable switch.
...
- mappings from organization A to Linear team X and organization B to Linear team Y
- one global git plugin install
- per-project workspace state stored under `project_workspace`

## 8.1 On-Disk Layout

Plugins live under the Rudder instance directory.

Suggested layout:

- `~/.rudder/instances/default/plugins/package.json`
- `~/.rudder/instances/default/plugins/node_modules/`
- `~/.rudder/instances/default/plugins/.cache/`
- `~/.rudder/instances/default/data/plugins/<plugin-id>/`
...
Rudder should add CLI commands:

- `pnpm rudder plugin list`
- `pnpm rudder plugin install <package[@version]>`
- `pnpm rudder plugin uninstall <plugin-id>`
- `pnpm rudder plugin upgrade <plugin-id> [version]`
- `pnpm rudder plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@rudderhq/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
Plugins may contribute tools that Rudder agents can use during runs.
...
- `executeTool(input)` — receives tool name, parsed parameters, and run context (agent ID, run ID, organization ID, project ID)
...
- Rudder server starts one worker process per installed plugin
...
Receives one typed Rudder domain event.
...
- context (organization id, project id, entity id, etc.)
...
- run context: agent ID, run ID, organization ID, project ID
...
- `orgId` — only receive events for a specific organization
- `agentId` — only receive events for a specific agent

Filters are optional. If omitted, the plugin receives all events of the subscribed type. Filters may be combined (e.g. filter by both organization and project).
...
```ts
ctx.events.on("plugin.@rudderhq/plugin-git.push-detected", async (event) => {
...
4. The plugin component uses the bridge to fetch data from its own worker (via `getData`), call actions (via `performAction`), read host context (current organization, project, entity), and use shared host UI primitives (design tokens, common components).
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@rudderhq/plugin-sdk/ui";

export function DashboardWidget({ context }: PluginWidgetProps) {
  const { data, loading } = usePluginData("sync-health", { orgId: context.orgId });
  ...
  return (
    <div>
      ...
      <button onClick={() => resync({ orgId: context.orgId })}>Resync Now</button>
    </div>
  );
}
```
...
2. The host mounts the plugin's `DashboardWidget` component into the dashboard widget slot, passing `context` (current organization, user, etc.) and the bridge.
...
- The host provides **design tokens and shared components** via `@rudderhq/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@rudderhq/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@rudderhq/plugin-sdk/ui` and their own dependencies.
...
## 19.2 Organization-Context Routes

- `/:orgPrefix/plugins/:pluginId`

These routes exist because the board UI is organized around organizations even though plugin installation is global.
...
- `/:orgPrefix/<entity>/:id?tab=<plugin-tab-id>`
...
- organization-context plugin pages

## 19.6 Shared Components In `@rudderhq/plugin-sdk/ui`
...
The `@rudderhq/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Rudder secret provider system rather than a plain text input
...
1. Core Rudder data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Rudder objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Rudder product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Rudder later decides git state is core product data, it should become a first-party table too
...
- `scope_kind` enum: `instance | organization | project | project_workspace | agent | issue | goal | run`
...
2. Secret refs resolve through the existing Rudder secret provider system.
...
## 24.3 Organization-Context Plugin Page

Each plugin may expose a organization-context main page:
...
6. The operator may force-purge immediately via CLI: `pnpm rudder plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Rudder server. This is a normative requirement, not optional.
...
### 27.1 `@rudderhq/plugin-test-harness`
...
```ts
import { createTestHarness } from "@rudderhq/plugin-test-harness";
...
const data = await harness.getData("sync-health", { orgId: "comp-1" });
expect(data.syncedCount).toBeGreaterThan(0);
```

### 27.2 Local Plugin Development

For developing a plugin against a running Rudder instance:

- The operator installs the plugin from a local path: `pnpm rudder plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-rudder-plugin`) that scaffolds:

- `package.json` with correct `rudderPlugin` keys
...
- `@rudderhq/plugin-workspace-files`
- `@rudderhq/plugin-terminal`
- `@rudderhq/plugin-git`
- `@rudderhq/plugin-linear`
- `@rudderhq/plugin-github-issues`
- `@rudderhq/plugin-grafana`
- `@rudderhq/plugin-runtime-processes`
- `@rudderhq/plugin-stripe`
...
- `@rudderhq/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@rudderhq/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@rudderhq/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@rudderhq/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@rudderhq/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@rudderhq/plugin-sdk/ui`
...
- `@rudderhq/plugin-test-harness`
- `create-rudder-plugin` starter template
...
Rudder should not implement a generic in-process hook bag modeled directly after local coding tools.

Rudder should implement:
...
That is the complete target design for the Rudder plugin system.
