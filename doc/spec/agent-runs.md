...
This spec defines how Rudder actually runs agents while staying runtime-agnostic.
...
1. Rudder is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Rudder manages lifecycle, and on exit Rudder parses JSON output and updates state.
...
7. Preserve organization scoping and existing governance invariants.
...
- `agents` with `agentRuntimeType` + `agentRuntimeConfig`.
...
6. `Realtime Event Hub`
   ...
   - Supports selective subscription by organization.
...
All tables remain organization-scoped.
...
One row per `(organization_id, agent_id, adapter_type, task_key)` for resumable session state.
...
Invariant: unique `(organization_id, agent_id, adapter_type, task_key)`.
...
- `run.startedAt`
- `heartbeat.reason`
- `rudder.skill` (shared Rudder skill text block)
...
2. Preferred transport is env vars (`RUDDER_*`) injected at runtime.
...
Primary transport: websocket channel per organization.

- Endpoint: `GET /api/orgs/:orgId/events/ws`
- Auth: board session or agent API key (organization-bound)
...
## 12.5 Recovery retry contract

Recovery retry is explicit run derivation, not a lossy wakeup shortcut.

1. Manual retry and automatic `process_lost` retry both create a recovery run from an existing run.
2. Recovery runs keep `retryOfRunId` and attach standardized `contextSnapshot.recovery` metadata.
...
1. Implement organization websocket hub.
...
8. All actions remain organization-scoped and auditable.
