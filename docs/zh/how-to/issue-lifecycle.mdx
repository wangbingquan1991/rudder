...
`issue` 是 Rudder 把人类意图变成可检查 agent 工作的主要方式。这篇指南关注实际用户旅程：什么时候创建 issue、什么时候分配执行者、什么时候加 reviewer、状态怎么设置，以及如何关闭循环。
...
Rudder 有意保持一个 issue 只有一个 assignee。如果 Growth、Engineering 和 QA 都要参与，就创建多个 issue 或子 issue。

## 什么时候加 reviewer

当关闭 issue 需要独立判断时，加 reviewer。
...
有输出可以判断时，再把 issue 移到 `in_review`，并让 reviewer 留下明确结论。
...
下面不是抽象任务板模板，而是从真实 Rudder 工作场景中脱敏改写出来的示例。阅读重点是路由判断：来了什么信号，应该沉淀成哪条 issue，下一步归谁，谁判断结果，以及什么证据可以关闭循环。
...
6. 只有当 issue 里有证据时才进 `in_review`：prompt 行为、runtime bridge、transcript 截图、测试证明原始下载命令已经消失。
7. Reviewer 接受行为和证据质量后，才能关闭为 `done`。

## 用例：Backlog 评论唤醒了错误 agent

一个 backlog issue 已经挂着 engineer assignee。Operator 评论时只 mention 了 designer，但 engineer 也被唤醒开始运行。这里的问题不只是通知噪音，而是 `backlog` 的含义被破坏了。
