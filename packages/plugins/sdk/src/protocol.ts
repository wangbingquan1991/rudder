 ...
 * channel.
 *
 * The Rudder plugin runtime uses JSON-RPC 2.0 over stdio to communicate
 ...
 * we use strings (UUIDs or monotonic counters) for all Rudder messages.
 */
...
/**
 * Rudder plugin-specific error codes.
 *
...
export interface InitializeParams {
  ...
  instanceInfo: {
    /** UUID of this Rudder instance. */
    instanceId: string;
    /** Semver version of the running Rudder host. */
    hostVersion: string;
  };
  /** Host API version. */
  apiVersion: number;
...
export interface WorkerToHostMethods {
  ...
  // Events
  "events.emit": [
    params: { name: string; orgId: string; payload: unknown },
    result: void,
  ];
  "events.subscribe": [
  ...
  "agents.invoke": [
    params: { agentId: string; orgId: string; prompt: string; reason?: string },
    result: { runId: string },
  ];

  // Agent Sessions
  "agents.sessions.create": [
    params: { agentId: string; orgId: string; taskKey?: string; reason?: string },
    result: { sessionId: string; agentId: string; orgId: string; status: "active" | "closed"; createdAt: string },
  ];
  "agents.sessions.list": [
    params: { agentId: string; orgId: string },
    result: Array<{ sessionId: string; agentId: string; orgId: string; status: "active" | "closed"; createdAt: string }>,
  ];
  "agents.sessions.sendMessage": [
    params: { sessionId: string; orgId: string; prompt: string; reason?: string },
    result: { runId: string },
  ];
  "agents.sessions.close": [
    params: { sessionId: string; orgId: string },
    result: void,
  ];
  ...
  "goals.get": [
    params: { goalId: string; orgId: string },
    result: Goal | null,
  ];
  "goals.create": [
    params: {
...
export interface WorkerToHostNotifications {
   ...
   * subscribed to the (pluginId, channel, orgId) tuple.
   *
   ...
   *
   * Emitted when the worker calls `ctx.streams.open(channel, orgId)`.
