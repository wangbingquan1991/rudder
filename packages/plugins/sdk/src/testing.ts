...
export interface TestHarness {
  ...
  ctx: PluginContext;
  /** Seed host entities for `ctx.organizations/projects/issues/agents/goals` reads. */
  seed(input: {
...
function allowsEvent(filter: EventFilter | undefined, event: PluginEvent): boolean {
  ...
  if (filter.orgId && filter.orgId !== String((event.payload as Record<string, unknown> | undefined)?.orgId ?? "")) return false;
...
function requireOrganizationId(orgId?: string): string {
  if (!orgId) throw new Error("orgId is required for this operation");
  return orgId;
}

function isInOrganization<T extends { orgId: string | null | undefined }>(
  record: T | null | undefined,
  orgId: string,
): record is T {
  return Boolean(record && record.orgId === orgId);
}
 ...
 * can validate plugin behavior without spinning up the Rudder server runtime.
 */
export function createTestHarness(options: TestHarnessOptions): TestHarness {
  ...
  const ctx: PluginContext = {
    ...
    events: {
      ...
      async emit(name, orgId, payload) {
        ...
        await harness.emit(`plugin.${manifest.id}.${name}`, payload, { orgId });
      },
    },
    jobs: {
      register(key, fn) {
    ...
    projects: {
      async list(input) {
        ...
        out = out.filter((project) => project.orgId === orgId);
        if (input?.offset) out = out.slice(input.offset);
        if (input?.limit) out = out.slice(0, input.limit);
        return out;
      },
      async get(projectId, orgId) {
        requireCapability(manifest, capabilitySet, "projects.read");
        const project = projects.get(projectId);
        return isInOrganization(project, orgId) ? project : null;
      },
      async listWorkspaces(projectId, orgId) {
        requireCapability(manifest, capabilitySet, "project.workspaces.read");
        if (!isInOrganization(projects.get(projectId), orgId)) return [];
        return projectWorkspaces.get(projectId) ?? [];
      },
      async getPrimaryWorkspace(projectId, orgId) {
        requireCapability(manifest, capabilitySet, "project.workspaces.read");
        if (!isInOrganization(projects.get(projectId), orgId)) return null;
        const workspaces = projectWorkspaces.get(projectId) ?? [];
        return workspaces.find((workspace) => workspace.isPrimary) ?? null;
      },
      async getWorkspaceForIssue(issueId, orgId) {
        requireCapability(manifest, capabilitySet, "project.workspaces.read");
        const issue = issues.get(issueId);
        if (!isInOrganization(issue, orgId)) return null;
    ...
    organizations: {
      async list(input) {
        requireCapability(manifest, capabilitySet, "organizations.read");
        let out = [...organizations.values()];
        if (input?.offset) out = out.slice(input.offset);
        if (input?.limit) out = out.slice(0, input.limit);
        return out;
      },
    ...
    issues: {
      ...
      async get(issueId, orgId) {
        requireCapability(manifest, capabilitySet, "issues.read");
        const issue = issues.get(issueId);
        return isInOrganization(issue, orgId) ? issue : null;
      },
      async create(input) {
      ...
      async update(issueId, patch, orgId) {
        requireCapability(manifest, capabilitySet, "issues.update");
        const record = issues.get(issueId);
        if (!isInOrganization(record, orgId)) throw new Error(`Issue not found: ${issueId}`);
        const updated: Issue = {
          ...record,
          ...patch,
          updatedAt: new Date(),
      ...
      async listComments(issueId, orgId) {
      ...
      async createComment(issueId, body, orgId) {
    ...
    agents: {
      async list(input) {
        ...
        out = out.filter((agent) => agent.orgId === orgId);
        if (input?.status) out = out.filter((agent) => agent.status === input.status);
        if (input?.offset) out = out.slice(input.offset);
        if (input?.limit) out = out.slice(0, input.limit);
        return out;
      },
      async get(agentId, orgId) {
        requireCapability(manifest, capabilitySet, "agents.read");
        const agent = agents.get(agentId);
        return isInOrganization(agent, orgId) ? agent : null;
      },
      async pause(agentId, orgId) {
        requireCapability(manifest, capabilitySet, "agents.pause");
        const cid = requireOrganizationId(orgId);
        const agent = agents.get(agentId);
        if (!isInOrganization(agent, cid)) throw new Error(`Agent not found: ${agentId}`);
      ...
      async invoke(agentId, orgId, opts) {
      ...
      sessions: {
        async create(agentId, orgId, opts) {
        ...
        async list(agentId, orgId) {
          requireCapability(manifest, capabilitySet, "agent.sessions.list");
          const cid = requireOrganizationId(orgId);
          return [...sessions.values()].filter(
            (s) => s.agentId === agentId && s.orgId === cid && s.status === "active",
          );
        },
        async sendMessage(sessionId, orgId, opts) {
          ...
          if (session.orgId !== orgId) throw new Error(`Session not found: ${sessionId}`);
          if (opts.onEvent) {
            sessionEventCallbacks.set(sessionId, opts.onEvent);
          }
          return { runId: randomUUID() };
        },
        async close(sessionId, orgId) {
    ...
    goals: {
      ...
      async get(goalId, orgId) {
        requireCapability(manifest, capabilitySet, "goals.read");
        const goal = goals.get(goalId);
        return isInOrganization(goal, orgId) ? goal : null;
      },
      async create(input) {
      ...
      async update(goalId, patch, orgId) {
    ...
    streams: (() => {
      const channelOrganizationMap = new Map<string, string>();
      return {
        open(channel: string, orgId: string) {
          channelOrganizationMap.set(channel, orgId);
        },
        emit(_channel: string, _event: unknown) {
          // No-op in test harness — events are not forwarded
        },
        close(channel: string) {
          channelOrganizationMap.delete(channel);
        },
      };
    })(),
    tools: {
  ...
  const harness: TestHarness = {
    ctx,
    seed(input) {
      for (const row of input.organizations ?? []) organizations.set(row.id, row);
    ...
    async executeTool<T = ToolResult>(name: string, params: unknown, runCtx: Partial<ToolRunContext> = {}) {
      ...
      const ctxToPass: ToolRunContext = {
        ...
        orgId: runCtx.orgId ?? "company-test",
