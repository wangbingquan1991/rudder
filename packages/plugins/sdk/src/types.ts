...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Rudder UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Rudder domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Rudder domain event or a plugin-namespaced event.
   *
   ...
   */
  emit(name: string, orgId: string, payload: unknown): Promise<void>;
}
...
export interface PluginSecretsClient {
   ...
   * in the Rudder secret provider (e.g. `"MY_API_KEY"`).
   *
...
export interface PluginProjectsClient {
   ...
   */
  list(input: { orgId: string; limit?: number; offset?: number }): Promise<Project[]>;
   ...
   * Requires the `projects.read` capability.
   */
  get(projectId: string, orgId: string): Promise<Project | null>;

  /**
   * List all workspaces attached to a project.
   *
   * @param projectId - UUID of the project
   * @param orgId - UUID of the company that owns the project
   * @returns All workspaces for the project, ordered with primary first
   */
  listWorkspaces(projectId: string, orgId: string): Promise<PluginWorkspace[]>;
   ...
   */
  getPrimaryWorkspace(projectId: string, orgId: string): Promise<PluginWorkspace | null>;
   ...
   * @param orgId - UUID of the company that owns the issue
...
/**
 * `ctx.organizations` — read company metadata.
 *
 * Requires `organizations.read` capability.
 */
export interface PluginOrganizationsClient {
  /**
   * List organizations visible to this plugin.
   */
  list(input?: { limit?: number; offset?: number }): Promise<Organization[]>;
...
export interface PluginIssueDocumentsClient {
   ...
   */
  list(issueId: string, orgId: string): Promise<IssueDocumentSummary[]>;
   ...
   * @param orgId - UUID of the company
   */
  get(issueId: string, key: string, orgId: string): Promise<IssueDocument | null>;
   ...
   * @param orgId - UUID of the company
   */
  delete(issueId: string, key: string, orgId: string): Promise<void>;
}
...
export interface PluginIssuesClient {
  ...
  get(issueId: string, orgId: string): Promise<Issue | null>;
  create(input: {
    orgId: string;
    projectId?: string;
    goalId?: string;
  ...
  listComments(issueId: string, orgId: string): Promise<IssueComment[]>;
  createComment(issueId: string, body: string, orgId: string): Promise<IssueComment>;
...
export interface PluginAgentsClient {
  list(input: { orgId: string; status?: Agent["status"]; limit?: number; offset?: number }): Promise<Agent[]>;
  get(agentId: string, orgId: string): Promise<Agent | null>;
  /** Pause an agent. Throws if agent is terminated or not found. Requires `agents.pause`. */
  pause(agentId: string, orgId: string): Promise<Agent>;
  /** Resume a paused agent (sets status to idle). Throws if terminated, pending_approval, or not found. Requires `agents.resume`. */
  resume(agentId: string, orgId: string): Promise<Agent>;
  ...
  invoke(agentId: string, orgId: string, opts: { prompt: string; reason?: string }): Promise<{ runId: string }>;
...
export interface PluginAgentSessionsClient {
  ...
  create(agentId: string, orgId: string, opts?: {
    taskKey?: string;
    reason?: string;
  }): Promise<AgentSession>;

  /** List active sessions for an agent owned by this plugin. Requires `agent.sessions.list`. */
  list(agentId: string, orgId: string): Promise<AgentSession[]>;
   ...
   */
  sendMessage(sessionId: string, orgId: string, opts: {
    prompt: string;
    reason?: string;
  ...
  close(sessionId: string, orgId: string): Promise<void>;
}
...
export interface PluginGoalsClient {
  ...
  get(goalId: string, orgId: string): Promise<Goal | null>;
  create(input: {
    orgId: string;
    title: string;
    description?: string;
 ...
 * real time via SSE.
 *
 * Streams are scoped to `(pluginId, channel, orgId)`. Multiple UI clients
 * can subscribe to the same channel concurrently.
 *
 * @example
 * ```ts
 * // Worker: stream chat tokens to the UI
 * ctx.streams.open("chat", orgId);
...
export interface PluginStreamsClient {
   ...
   */
  open(channel: string, orgId: string): void;
 ...
 *       const state = await ctx.state.get({ scopeKind: "company", scopeId: String(orgId), stateKey: "last-sync" });
...
export interface PluginContext {
  ...
  /** Read company metadata. Requires `organizations.read`. */
  organizations: PluginOrganizationsClient;
