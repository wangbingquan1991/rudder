 ...
 * import { startWorkerRpcHost } from "@rudderhq/plugin-sdk";
 *
 * startWorkerRpcHost({ plugin });
 * ```
 *
...
export function startWorkerRpcHost(options: WorkerRpcHostOptions): WorkerRpcHost {
  ...
  const toolHandlers = new Map<string, {
    declaration: Pick<import("@rudderhq/shared").PluginToolDeclaration, "displayName" | "description" | "parametersSchema">;
  ...
  function buildContext(): PluginContext {
    return {
      ...
      events: {
        ...
        async emit(name: string, orgId: string, payload: unknown): Promise<void> {
          await callHost("events.emit", { name, orgId, payload });
        },
      },
      ...
      projects: {
        ...
        async get(projectId: string, orgId: string) {
          return callHost("projects.get", { projectId, orgId });
        },

        async listWorkspaces(projectId: string, orgId: string) {
          return callHost("projects.listWorkspaces", { projectId, orgId });
        },

        async getPrimaryWorkspace(projectId: string, orgId: string) {
          return callHost("projects.getPrimaryWorkspace", { projectId, orgId });
        },

        async getWorkspaceForIssue(issueId: string, orgId: string) {
          return callHost("projects.getWorkspaceForIssue", { issueId, orgId });
        },
      },

      organizations: {
        async list(input) {
          return callHost("organizations.list", {
            limit: input?.limit,
            offset: input?.offset,
          });
        },
        ...
        async get(orgId: string) {
          return callHost("organizations.get", { orgId });
        },
      },
      ...
      issues: {
        ...
        async get(issueId: string, orgId: string) {
          return callHost("issues.get", { issueId, orgId });
        },
        ...
        async update(issueId: string, patch, orgId: string) {
        ...
        async listComments(issueId: string, orgId: string) {
          return callHost("issues.listComments", { issueId, orgId });
        },

        async createComment(issueId: string, body: string, orgId: string) {
      ...
      agents: {
        ...
        async get(agentId: string, orgId: string) {
          return callHost("agents.get", { agentId, orgId });
        },

        async pause(agentId: string, orgId: string) {
          return callHost("agents.pause", { agentId, orgId });
        },

        async resume(agentId: string, orgId: string) {
          return callHost("agents.resume", { agentId, orgId });
        },

        async invoke(agentId: string, orgId: string, opts: { prompt: string; reason?: string }) {
          return callHost("agents.invoke", { agentId, orgId, prompt: opts.prompt, reason: opts.reason });
        },

        sessions: {
          async create(agentId: string, orgId: string, opts?: { taskKey?: string; reason?: string }) {
          ...
          async list(agentId: string, orgId: string) {
            return callHost("agents.sessions.list", { agentId, orgId });
          },

          async sendMessage(sessionId: string, orgId: string, opts: {
            prompt: string;
            reason?: string;
          ...
          async close(sessionId: string, orgId: string) {
            sessionEventCallbacks.delete(sessionId);
            await callHost("agents.sessions.close", { sessionId, orgId });
          },
        },
      },
      ...
      goals: {
        ...
        async get(goalId: string, orgId: string) {
          return callHost("goals.get", { goalId, orgId });
        },
      ...
      streams: (() => {
        // Track channel → orgId so emit/close don't require orgId
        ...
        return {
          open(channel: string, orgId: string): void {
            ...
            notifyHost("streams.open", { channel, orgId });
          },
          emit(channel: string, event: unknown): void {
            const orgId = channelOrganizationMap.get(channel) ?? "";
            notifyHost("streams.emit", { channel, orgId, event });
          },
          close(channel: string): void {
            const orgId = channelOrganizationMap.get(channel) ?? "";
            channelOrganizationMap.delete(channel);
            notifyHost("streams.close", { channel, orgId });
          },
        };
      })(),
