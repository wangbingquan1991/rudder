...
Notes:
  - Stable releases publish the committed workspace semver under npm dist-tag
    "latest" and create git tag v<version>.
...
COMMITTED_STABLE_VERSION=""
PUBLIC_PACKAGE_VERSION_MISMATCH=""
...
while IFS=$'\t' read -r _pkg_dir pkg_name pkg_version; do
  [ -n "$pkg_name" ] || continue
  if [ -z "$COMMITTED_STABLE_VERSION" ]; then
    COMMITTED_STABLE_VERSION="$pkg_version"
    continue
  fi

  if [ "$pkg_version" != "$COMMITTED_STABLE_VERSION" ]; then
    if [ -n "$PUBLIC_PACKAGE_VERSION_MISMATCH" ]; then
      PUBLIC_PACKAGE_VERSION_MISMATCH="${PUBLIC_PACKAGE_VERSION_MISMATCH}; "
    fi
    PUBLIC_PACKAGE_VERSION_MISMATCH="${PUBLIC_PACKAGE_VERSION_MISMATCH}${pkg_name}@${pkg_version}"
  fi
done <<< "$PUBLIC_PACKAGE_INFO"
...
release_info "  Committed stable version: $TARGET_STABLE_VERSION"
if [ "$channel" = "canary" ]; then
...
release_info ""
if [ "$channel" = "canary" ]; then
  release_info "==> Step 3/7: Rewriting workspace versions for canary payload..."
...
if [ "$dry_run" = true ]; then
  ...
  if [ "$channel" = "canary" ]; then
    ...
    release_info "Install with: npx @rudderhq/cli@canary onboard"
