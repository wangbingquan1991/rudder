...
describe("codex_local stale session detection", () => {
  ...
  it("treats missing thread rollout as an unknown session error", () => {
    const stderr =
      "Error: thread/resume: thread/resume failed: no rollout found for thread id 019dc96b-3624-7ce1-8fb5-bd05d3f50afd";
  ...
  it("parses turn and reasoning lifecycle events", () => {
    ...
    expect(
      parseCodexStdoutLine(
        JSON.stringify({
          type: "item.completed",
          item: { id: "item_1", type: "reasoning", text: "**Preparing to use rudder skill**" },
        }),
        ts,
      ),
    ).toEqual([
