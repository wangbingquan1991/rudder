...
function readIssueRefsFromContextLinks(value: unknown): string[] {
  ...
  for (const item of value) {
    ...
    if (!link) continue;
    if (readString(link.entityType) !== "issue") continue;
    const issueId = readString(link.entityId);
    if (issueId) refs.push(issueId);
  }
  return refs;
}

function buildIssueRefsForPayload(payload: Record<string, unknown>): Set<string> {
  ...
  if (entityType === "chat") {
    const convertedIssueId = readString(details?.issueId);
    if (convertedIssueId) refs.add(convertedIssueId);

    if (readString(details?.entityType) === "issue") {
      const linkedIssueId = readString(details?.entityId);
      if (linkedIssueId) refs.add(linkedIssueId);
    }

    for (const ref of readIssueRefsFromContextLinks(details?.contextLinks)) {
      refs.add(ref);
    }
  }
...
function resolveAgentName(
  ...
  const agents = queryClient.getQueryData<Agent[]>(queryKeys.agents.list(orgId));
...
function resolveIssueQueryRefs(
  ...
  const listIssues = queryClient.getQueryData<Issue[]>(queryKeys.issues.list(orgId));
...
function resolveIssueToastContext(
  ...
  const issueRefs = resolveIssueQueryRefs(queryClient, orgId, issueId, details);
  ...
  const listIssue = queryClient
    .getQueryData<Issue[]>(queryKeys.issues.list(orgId))
...
function shouldSuppressActivityToastForVisibleIssue(
  ...
  if (issueRefs.size === 0) return false;

  const context = resolveVisibleIssueRouteContext(queryClient, pathname, options);
  if (!context) return false;

  return overlaps(context.issueRefs, issueRefs);
}

function resolveVisibleChatConversationId(
  pathname: string,
  options?: VisibleRouteOptions,
): string | null {
  const isForegrounded = options?.isForegrounded ?? isPageForegrounded();
  ...
  const relativePath = toOrganizationRelativePath(pathname);
  const segments = relativePath.split("/").filter(Boolean);
  if (segments[0] !== "chat" && !(segments[0] === "messenger" && segments[1] === "chat")) return null;
  if (segments[0] === "messenger" && segments[1] === "chat") {
...
function shouldSuppressChatToastForVisibleConversation(
  pathname: string,
  payload: Record<string, unknown>,
  options?: VisibleRouteOptions,
): boolean {
  const visibleConversationId = resolveVisibleChatConversationId(pathname, options);
...
function buildJoinRequestToast(
  ...
  return {
    ...
    tone: "info",
    action: { label: "Open Messenger", href: "/messenger" },
    dedupeKey: `join-request:${entityId}`,
  };
}

function buildChatToast(
  payload: Record<string, unknown>,
): ToastInput | null {
  const entityType = readString(payload.entityType);
  const action = readString(payload.action);
  const entityId = readString(payload.entityId);
  const details = readRecord(payload.details);
  if (entityType !== "chat" || action !== "chat.message_added" || !entityId || !details) return null;

  const role = readString(details.role);
  if (role === "user") return null;

  const preview = readString(details.preview);
  const kind = readString(details.kind);
  const title =
    kind === "issue_proposal"
      ? "Chat proposed an issue"
      : kind === "operation_proposal"
        ? "Chat proposed a lightweight change"
        : "New chat reply";

  return {
    title,
    body: preview ? truncate(preview.replace(/\n/g, " "), 96) : undefined,
    tone: "info",
    action: { label: "Open chat", href: `/messenger/chat/${entityId}` },
    dedupeKey: `chat-message:${entityId}:${readString(details.messageId) ?? "na"}`,
  };
}
...
function invalidateActivityQueries(
  ...
  if (entityType === "project") {
    queryClient.invalidateQueries({ queryKey: queryKeys.projects.list(orgId) });
  ...
  if (entityType === "cost_event") {
    queryClient.invalidateQueries({ queryKey: queryKeys.costs(orgId) });
    queryClient.invalidateQueries({ queryKey: queryKeys.usageByProvider(orgId) });
    queryClient.invalidateQueries({ queryKey: queryKeys.usageWindowSpend(orgId) });
  ...
  if (entityType === "automation" || entityType === "automation_trigger" || entityType === "automation_run") {
...
export function LiveUpdatesProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      const url = `${protocol}://${window.location.host}/api/orgs/${encodeURIComponent(selectedOrganizationId)}/events/ws`;
