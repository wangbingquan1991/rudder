...
function findAtomicInlineNodeInSubtree(
  ...
  const childGetter = direction === "backward"
    ? () => node.getLastChild()
    : () => node.getFirstChild();
  let child = childGetter();
  while (child) {
    const match = findAtomicInlineNodeInSubtree(child, direction);
    if (match) return match;
    child = direction === "backward" ? child.getPreviousSibling() : child.getNextSibling();
  }
...
function findMentionLinkNode(node: LexicalNode | null | undefined): LinkNode | MentionTokenNode | SkillTokenNode | null {
  ...
  while (parent) {
    if (isAtomicInlineNode(parent)) return parent;
...
function findMentionLinkNodeAtPoint(
  ...
  if (point.type === "element" && $isElementNode(node)) {
    ...
    return findAtomicInlineNodeInSubtree(node.getChildAtIndex(childIndex), direction);
  }
