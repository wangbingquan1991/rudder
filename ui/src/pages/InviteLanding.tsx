...
export function InviteLandingPage() {
  ...
  const [agentRuntimeType, setAdapterType] = useState<AgentRuntimeType>("claude_local");
  ...
  const joinTypeLabels: Record<JoinType, string> = useMemo(
    () => ({
      human: t("inviteLanding.joinType.human"),
      agent: t("inviteLanding.joinType.agent"),
    }),
    [t],
  );
  ...
  const acceptMutation = useMutation({
    mutationFn: async () => {
      if (!invite) throw new Error(t("inviteLanding.notAvailable.title"));
    ...
    onError: (err) => {
      setError(err instanceof Error ? err.message : t("inviteLanding.error.failedToAccept"));
    },
  });
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">{t("inviteLanding.notAvailable.title")}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {t("inviteLanding.notAvailable.description")}
          </p>
        </div>
      </div>
    );
  ...
  if (result?.kind === "bootstrap") {
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">{t("inviteLanding.bootstrapComplete.title")}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {t("inviteLanding.bootstrapComplete.description")}
          </p>
  ...
  if (result?.kind === "join") {
    ...
    return (
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          <h1 className="text-lg font-semibold">{t("inviteLanding.joinSubmitted.title")}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {t("inviteLanding.joinSubmitted.description")}
          </p>
          <div className="mt-4 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
            {t("inviteLanding.requestId")} <span className="font-mono">{payload.id}</span>
          </div>
          {claimSecret && claimApiKeyPath && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">{t("inviteLanding.claimSecret.title")}</p>
          ...
          {(onboardingSkillUrl || onboardingSkillPath || onboardingInstallPath) && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">{t("inviteLanding.skillBootstrap.title")}</p>
          ...
          {(onboardingTextUrl || onboardingTextPath) && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">{t("inviteLanding.onboardingText.title")}</p>
          ...
          {diagnostics.length > 0 && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">{t("inviteLanding.connectivityDiagnostics.title")}</p>
              {diagnostics.map((diag, idx) => (
                <div key={`${diag.code}:${idx}`} className="space-y-0.5">
                  <p className={diag.level === "warn" ? "text-amber-600 dark:text-amber-400" : undefined}>
                    [{t(diag.level === "warn" ? "inviteLanding.diagLevel.warn" : "inviteLanding.diagLevel.info")}] {diag.message}
                  </p>
    ...
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        <h1 className="text-xl font-semibold">
          {invite.inviteType === "bootstrap_ceo" ? t("inviteLanding.bootstrap.title") : t("inviteLanding.join.title")}
        </h1>
        <p className="mt-2 text-sm text-muted-foreground">{t("inviteLanding.expiresAt", { date: dateTime(invite.expiresAt) })}</p>
        ...
        {invite.inviteType !== "bootstrap_ceo" && (
          <div className="mt-5 flex gap-2">
            {availableJoinTypes.map((type) => (
              ...
              >
                {t("inviteLanding.joinAs", { type: joinTypeLabels[type] })}
              </button>
            ))}
          </div>
        )}
        ...
        {joinType === "agent" && invite.inviteType !== "bootstrap_ceo" && (
          <div className="mt-4 space-y-3">
            <label className="block text-sm">
              <span className="mb-1 block text-muted-foreground">{t("inviteLanding.field.agentName")}</span>
              <input
            ...
            <label className="block text-sm">
              <span className="mb-1 block text-muted-foreground">{t("inviteLanding.field.runtimeType")}</span>
              <select
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
                value={agentRuntimeType}
                onChange={(event) => setAdapterType(event.target.value as AgentRuntimeType)}
              >
                {joinAdapterOptions.map((type) => (
                  <option key={type} value={type} disabled={!ENABLED_INVITE_ADAPTERS.has(type)}>
                    {adapterLabels[type]}{!ENABLED_INVITE_ADAPTERS.has(type) ? ` (${t("inviteLanding.runtime.comingSoon")})` : ""}
                  </option>
                ))}
              </select>
            </label>
            <label className="block text-sm">
              <span className="mb-1 block text-muted-foreground">{t("inviteLanding.field.capabilities")}</span>
              <textarea
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
                rows={4}
                value={capabilities}
        ...
        {requiresAuthForHuman && (
          <div className="mt-4 rounded-md border border-border bg-muted/30 p-3 text-sm">
            {t("inviteLanding.requiresAuth")}
            <div className="mt-2">
              <Button asChild size="sm" variant="outline">
                <Link to={`/auth?next=${encodeURIComponent(`/invite/${token}`)}`}>{t("inviteLanding.signInAction")}</Link>
              </Button>
            </div>
          </div>
        )}
        ...
        >
          {acceptMutation.isPending
            ? t("inviteLanding.submit.submitting")
            : invite.inviteType === "bootstrap_ceo"
              ? t("inviteLanding.submit.bootstrap")
              : t("inviteLanding.submit.join")}
        </Button>
      </div>
    </div>
  );
