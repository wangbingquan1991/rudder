...
export function OrgChart() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: "Organization Structure" }]);
  }, [setBreadcrumbs]);
  ...
  if (!selectedOrganizationId) {
    return <EmptyState icon={Network} message="Select an organization to view the Organization Structure." />;
  }
  ...
  if (orgTreeError || agentsError) {
    const message = (orgTreeError instanceof Error ? orgTreeError.message : null)
      ?? (agentsError instanceof Error ? agentsError.message : "Failed to load organization structure.");
    return <p className="text-sm text-destructive">{message}</p>;
  }

  if (orgTree && orgTree.length === 0) {
    return <EmptyState icon={Network} message="No Organization Structure defined." />;
  }
