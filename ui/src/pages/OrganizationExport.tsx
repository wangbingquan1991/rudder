...
/**
 * Filter .rudder.yaml content so it only includes entries whose
...
function filterPaperclipYaml(yaml: string, checkedFiles: Set<string>): string {
  ...
  const filterableSections = new Set(["agents", "projects", "tasks", "automations"]);
...
function downloadZip(
  exported: OrganizationPortabilityExportResult,
  selectedFiles: Set<string>,
  effectiveFiles: Record<string, OrganizationPortabilityFileEntry>,
) {
  const filteredFiles: Record<string, OrganizationPortabilityFileEntry> = {};
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm rudder organization import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [Rudder](https://github.com/Undertone0809/rudder) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [Rudder](https://github.com/Undertone0809/rudder) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
function filePathFromLocation(pathname: string): string | null {
  const marker = "/organization/export/files/";
...
export function OrganizationExport() {
  ...
  const [exportData, setExportData] = useState<OrganizationPortabilityExportPreviewResult | null>(null);
  ...
  const selectFile = useCallback(
    (filePath: string | null, replace = false) => {
      ...
      if (filePath) {
        navigate(`/organization/export/files/${encodeURI(filePath)}`, { replace });
      } else {
        navigate("/organization/export", { replace });
      }
    },
    [navigate],
  );
  ...
  const exportPreviewMutation = useMutation({
    mutationFn: () =>
      organizationsApi.exportPreview(selectedOrganizationId!, {
  ...
  useEffect(() => {
    if (!selectedOrganizationId || exportPreviewMutation.isPending) return;
    if (!isSessionFetched || !areAgentsFetched || !areProjectsFetched) return;
    setExportData(null);
    exportPreviewMutation.mutate();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedOrganizationId, isSessionFetched, areAgentsFetched, areProjectsFetched, sidebarOrderKey]);
  ...
  // Recompute .rudder.yaml and README.md content whenever checked files
  // change so the preview & download always reflect the current selection.
  const effectiveFiles = useMemo(() => {
    if (!exportData) return {} as Record<string, OrganizationPortabilityFileEntry>;
    const filtered = { ...exportData.files };

    // Filter .rudder.yaml
    const yamlPath = exportData.rudderExtensionPath;
    ...
    if (typeof exportData.files["README.md"] === "string") {
      const organizationName = exportData.manifest.organization?.name ?? selectedOrganization?.name ?? "Organization";
      const organizationDescription = exportData.manifest.organization?.description ?? null;
    ...
    return filtered;
  }, [exportData, checkedFiles, selectedOrganization?.name]);
  ...
  if (!selectedOrganizationId) {
    return <EmptyState icon={Package} message="Select a organization to export." />;
  }
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-4 text-sm">
            <span className="font-medium">
              {selectedOrganization?.name ?? "Organization"} export
            </span>
