...
function buildActionMap(preview: OrganizationPortabilityPreviewResult): Map<string, string> {
  ...
  if (manifest.organization) {
    const path = ensureMarkdownPath(manifest.organization.path);
    map.set(
      path,
      preview.plan.organizationAction === "none" ? "skip" : preview.plan.organizationAction,
    );
  }
...
function ImportPreviewPane({
  ...
  content: OrganizationPortabilityFileEntry | null;
  allFiles: Record<string, OrganizationPortabilityFileEntry>;
  action: string | null;
...
function applyImportedSidebarOrder(
  preview: OrganizationPortabilityPreviewResult | null,
  result: {
  ...
  if (orderedAgentIds.length > 0) {
    writeAgentOrder(getAgentOrderStorageKey(result.organization.id, userId), orderedAgentIds);
  }
  if (orderedProjectIds.length > 0) {
    writeProjectOrder(getProjectOrderStorageKey(result.organization.id, userId), orderedProjectIds);
  }
}
...
function AdapterPickerList({
  ...
  agentRuntimeOverrides: Record<string, string>;
  expandedSlugs: Set<string>;
  configValues: Record<string, CreateConfigValues>;
  onChangeAdapter: (slug: string, agentRuntimeType: string) => void;
  ...
  return (
    <div className="mx-5 mt-3">
      <div className="rounded-md border border-border">
        ...
        <div className="divide-y divide-border">
          {agents.map((agent) => {
            const selectedType = agentRuntimeOverrides[agent.slug] ?? agent.agentRuntimeType;
            const isExpanded = expandedSlugs.has(agent.slug);
            const vals = configValues[agent.slug] ?? { ...defaultCreateValues, agentRuntimeType: selectedType };
...
export function OrganizationImport() {
  ...
  const [importPreview, setImportPreview] =
    useState<OrganizationPortabilityPreviewResult | null>(null);
  ...
  const [collisionStrategy, setCollisionStrategy] = useState<OrganizationPortabilityCollisionStrategy>("rename");

  // Adapter override state
  const [agentRuntimeOverrides, setAdapterOverrides] = useState<Record<string, string>>({});
  const [adapterExpandedSlugs, setAdapterExpandedSlugs] = useState<Set<string>>(new Set());
  const [agentRuntimeConfigValues, setAdapterConfigValues] = useState<Record<string, CreateConfigValues>>({});

  // Fetch current organization agents to find CEO adapter type
  const { data: organizationAgents } = useQuery({
    queryKey: selectedOrganizationId ? queryKeys.agents.list(selectedOrganizationId) : ["agents", "none"],
    queryFn: () => agentsApi.list(selectedOrganizationId!),
    enabled: Boolean(selectedOrganizationId),
  });
  const ceoAdapterType = useMemo(() => {
    if (!organizationAgents) return "claude_local";
    const ceo = organizationAgents.find((a) => a.role === "ceo");
    return ceo?.agentRuntimeType ?? "claude_local";
  }, [organizationAgents]);

  const localZipHelpText =
    "Upload a .zip exported directly from Rudder. Re-zipped archives created by Finder, Explorer, or other zip tools may not import correctly.";
  ...
  function buildSource(): OrganizationPortabilitySource | null {
    if (sourceMode === "local") {
  ...
  const previewMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      // Check all files by default, then uncheck ORGANIZATION.md for existing organization
      const allFiles = new Set(Object.keys(result.files));
      if (targetMode === "existing" && result.manifest.organization && result.plan.organizationAction === "update") {
        const organizationPath = ensureMarkdownPath(result.manifest.organization.path);
        allFiles.delete(organizationPath);
      }
  ...
  const importMutation = useMutation({
    mutationFn: () => {
      const source = buildSource();
      if (!source) throw new Error("No source configured.");
      return organizationsApi.importBundle({
        source,
        include: { organization: true, agents: true, projects: true, issues: true },
        target:
          targetMode === "new"
        ...
        agentRuntimeOverrides: buildFinalAdapterOverrides(),
      });
    },
    onSuccess: async (result) => {
      await queryClient.invalidateQueries({ queryKey: queryKeys.organizations.all });
      const importedOrganization = await organizationsApi.get(result.organization.id);
      ...
      setSelectedOrganizationId(importedOrganization.id);
      pushToast({
        tone: "success",
  ...
  function handleAdapterChange(slug: string, agentRuntimeType: string) {
    setAdapterOverrides((prev) => ({ ...prev, [slug]: agentRuntimeType }));
  ...
  function handleAdapterConfigChange(slug: string, patch: Partial<CreateConfigValues>) {
    setAdapterConfigValues((prev) => ({
      ...prev,
      [slug]: { ...(prev[slug] ?? { ...defaultCreateValues, agentRuntimeType: agentRuntimeOverrides[slug] ?? "claude_local" }), ...patch },
    }));
  }
  ...
  const adapterAgents = useMemo<AdapterPickerItem[]>(() => {
    ...
    return importPreview.manifest.agents.map((a) => ({
      slug: a.slug,
      name: a.name,
      agentRuntimeType: a.agentRuntimeType,
    }));
  }, [importPreview]);

  // Build final agentRuntimeOverrides for import request
  function buildFinalAdapterOverrides(): Record<string, OrganizationPortabilityAgentRuntimeOverride> | undefined {
    if (adapterAgents.length === 0) return undefined;
    const overrides: Record<string, OrganizationPortabilityAgentRuntimeOverride> = {};
    for (const agent of adapterAgents) {
      const selectedType = agentRuntimeOverrides[agent.slug] ?? agent.agentRuntimeType;
      const configVals = agentRuntimeConfigValues[agent.slug];
      const override: OrganizationPortabilityAgentRuntimeOverride = { agentRuntimeType: selectedType };
      if (configVals) {
        const uiAdapter = getUIAdapter(selectedType);
        override.agentRuntimeConfig = uiAdapter.buildAdapterConfig(configVals);
      }
  ...
  if (!selectedOrganizationId) {
    return <EmptyState icon={Download} message="Select a organization to import into." />;
  }
    ...
    <div>
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        ...
        {sourceMode === "local" ? (
          ...
          <Field
            label="GitHub URL"
            hint="Repo tree path or blob URL to ORGANIZATION.md (e.g. github.com/owner/repo/tree/main/organization)."
          >
            <input
              className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              type="text"
              value={importUrl}
              placeholder="https://github.com/owner/repo/tree/main/organization"
              onChange={(e) => {
        ...
        <Field label="Target" hint="Import into this organization or create a new one.">
          ...
          >
            ...
            <option value="existing">
              Existing organization: {selectedOrganization?.name}
            </option>
          </select>
        </Field>
        ...
        >
          <select
            ...
            onChange={(e) => {
              setCollisionStrategy(e.target.value as OrganizationPortabilityCollisionStrategy);
              setImportPreview(null);
            }}
          >
      ...
      {importPreview && (
        <>
          ...
          <AdapterPickerList
            ...
            configValues={agentRuntimeConfigValues}
