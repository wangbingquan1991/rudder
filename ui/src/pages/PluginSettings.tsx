 ...
 *
 * URL params:
 * - `orgPrefix` — the organization slug (for breadcrumb links).
...
export function PluginSettings() {
  ...
  const { orgPrefix, pluginId } = useParams<{ orgPrefix?: string; pluginId: string }>();
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedOrganization?.name ?? "Organization", href: "/dashboard" },
      { label: "Settings", href: "/instance/settings/heartbeats" },
      { label: "Plugins", href: "/instance/settings/plugins" },
      { label: plugin?.manifestJson?.displayName ?? plugin?.packageName ?? "Plugin Details" },
    ]);
  }, [selectedOrganization?.name, setBreadcrumbs, orgPrefix, plugin]);
  ...
  if (!plugin) {
    return <Navigate to="/instance/settings/plugins" replace state={overlayState} />;
  }
