...
const surfaceOptions: Array<{
...
}> = [
  {
    ...
    description: "The long-form run page with the `Nice | Raw` toggle and the full execution timeline / transcript review surface.",
    icon: MonitorCog,
  },
  {
    id: "live",
    ...
    <div className="overflow-hidden rounded-[28px] border border-border/70 bg-[linear-gradient(180deg,rgba(8,145,178,0.04),transparent_28%),var(--background)] shadow-[0_24px_60px_rgba(15,23,42,0.10)]">
      ...
      <div className="space-y-4 px-5 py-5">
        ...
        <div className="rounded-[20px] bg-background/90 px-5 py-4 shadow-[0_10px_24px_rgba(15,23,42,0.06)]">
          ...
          <div className="text-sm leading-7 text-foreground/88">
            codex CLI 本体在 PATH 里，而且版本与子命令都能读到。最后我再做一次真正的非交互调用，确认不是只有帮助页可用。
          </div>
        </div>
      </div>
    </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[260px_minmax(0,1fr)]">
          <aside className="border-b border-border/60 bg-background/75 p-5 lg:border-b-0 lg:border-r">
            <div className="mb-5">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                Built from a real Rudder development run, then sanitized so no secrets, local paths, or environment details survive into the fixture.
              </p>
            </div>
          ...
          <main className="min-w-0 p-5">
            ...
            {selectedSurface === "detail" ? (
              ...
              <div className={cn(density === "compact" && "max-w-4xl")}>
                <ChatPreview streaming={streaming} mode={detailMode} density={density} />
              </div>
            ) : (
